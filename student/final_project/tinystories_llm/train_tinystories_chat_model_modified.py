# Modified for custom instruction tuning JSON datasets
import os
import torch
import argparse
...
def train_and_evaluate(args):
    ...
    # Load dataset (MODIFIED FOR INPUTTING SINGLE INSTRUCTION JSON FILE)
    print(f"Loading dataset: {args.dataset}")
    dataset = load_dataset('json', data_files={'train': args.dataset})
    dataset = dataset['train'].train_test_split(test_size=0.2) # Creating train/val split
