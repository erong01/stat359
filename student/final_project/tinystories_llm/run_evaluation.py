...
# Copied directly from the original code in main()
def generate_response(model, tokenizer, device, prompt, args, user_token_id, assistant_token_id):
...
def run_evaluation(model, tokenizer, device, args, user_token_id, assistant_token_id):
    '''Generates the response outputs file from the prompts file.'''
    # Load prompts file
...
def main():
    ...
    # Modify to run evaluation insteaf of interactive chat if prompt file is given
    if args.prompt_file is not None:
        run_evaluation(model, tokenizer, device, args, user_token_id, assistant_token_id)
        return
