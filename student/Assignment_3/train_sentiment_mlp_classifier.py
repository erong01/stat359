...
# Revise get_sentence_embedding function since only one embedding model is used here
def get_sentence_embedding(sentence, model):
    ...
    if len(word_vecs) == 0:
        # Embedding dim = model.vector_size = 300
        return np.zeros(model.vector_size)
    
    return np.mean(word_vecs, axis=0)

print("\n========== Encoding Sentences as Mean-Word Embeddings ==========")
X = np.array([get_sentence_embedding(sentence, embedding_model) for sentence in data['sentence']])
y = data['label'].values
...
model = MLPClassifier(input_dim, num_classes, dropout=0.4)
