...
(Used the train_sentiment_rnn_classifier.py script as a start.)
"""
...
# Assuming that the .model and .model.vector.npy files already exist in the directory
embedding_model = KeyedVectors.load('fasttext-wiki-news-subwords-300.model', mmap='r')
print("Model loaded.")

def get_device():
    return "mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"

# Reusing some code from Assignment 2
def tokenize_text(text):
    return word_tokenize(text.lower())

def get_padded_seq(sentence, model, max_len=32):
    ...
    for token in tokens:
        # Skipping OOV words
        if token in model:
            seq.append(model[token])

    # If no tokens found, use a small random vector to avoid entire sequences of zeroes
    if len(seq) == 0:
        seq.append(np.random.normal(scale=0.001, size=(emb_dim,)))
...
print("\n========== Encoding Sentences as FastText Sequences ==========")
max_seq_len = 32
X_seq = np.array([get_padded_seq(sentence, embedding_model, max_seq_len) for sentence in data['sentence']])
y = data['label'].values
...
class LSTMClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_classes, dropout=0.5):
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True, 
            dropout=dropout if num_layers > 1 else 0    # To avoid warning when num_layers=1
        )
...
hidden_dim = 128
num_layers = 1
model = LSTMClassifier(input_dim, hidden_dim, num_layers, num_classes, dropout=0.3)
